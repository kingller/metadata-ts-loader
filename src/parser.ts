...
export class Parser {
    ...
    public getPropsInfo(
        ...
        propsMap.forEach((propsObj, propName) => {
            ...
            const propType = this.checker.getTypeOfSymbolAtLocation(propsObj, propsObj.valueDeclaration!);
    ...
    public getFullJsDocComment(symbol: ts.Symbol): JSDoc {
        ...
        tags.forEach((tag) => {
            let trimmedText = '';
            const tagText = (tag.text as any) as { text: string; kind: string }[] | string;
...
function getTextValueOfFunctionProperty(exp: ts.Symbol, source: ts.SourceFile, propertyName: string) {
    const [textValue] = source.statements
        ...
        .filter((statement) => {
            ...
            return (
                ((expr.left as ts.PropertyAccessExpression).expression as ts.Identifier).escapedText === exp.getName()
            );
        })
        .filter((statement) => {
        ...
        .map((statement) => {
            return (((statement as ts.ExpressionStatement).expression as ts.BinaryExpression).right as ts.Identifier)
                .text;
        });
        ...
        .reduce<ComponentDoc[]>((docs, sourceFile) => {
            ...
            Array.prototype.push.apply(
                ...
                checker
                    ...
                    .map((exp) => parser.getInterfaceInfo(exp, sourceFile, parserOpts.componentNameResolver))
